...
h0successrate <- totalpizza / (ftotalpizza + totalpizza)
h0failurerate <- 1 - h0successrate

pizzatimes <- c()
fpizzatimes <- c()
for (i in 0:11) {
  pizzatimes <- c(pizzatimes, sum(clean_data$successes[hour >= (2 * i) & hour < (2 * i) + 2]))
  fpizzatimes <- c(fpizzatimes, sum(clean_data$failures[hour >= (2 * i) & hour < (2 * i) + 2]))
}

pizzatotalstwo <- c()
for (i in 1:length(pizzatimes)) {
  pizzatotalstwo <- c(pizzatotalstwo, pizzatimes[i] + fpizzatimes[i])
}

csqtwo <- 0
for (i in 1:length(pizzatimes)) {
  s_expected <- pizzatotalstwo[i] * h0successrate
  f_expected <- pizzatotalstwo[i] * h0failurerate
  csqtwo <- csqtwo + ((pizzatimes[i] - s_expected) ^ 2) / s_expected
  csqtwo <- csqtwo + ((fpizzatimes[i] - f_expected) ^ 2) / f_expected
}

prob_timeofday_twohr <- pchisq(csqtwo, 11, lower.tail = FALSE)
